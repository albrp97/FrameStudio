...
DEFAULT_FPS_PYTHON = Path.home() / ".cache" / "resolve-fps" / "trt" / "bin" / "python"
DEFAULT_FPS_SITE = (
    Path.home() / ".cache" / "resolve-fps" / "trt" / "lib" / "python3.13" / "site-packages"
)
DEFAULT_GRAPH = Path(__file__).resolve().parent / "benchmarks" / "fps.vpy"
DEFAULT_TRT_CACHE = Path.home() / ".cache" / "resolve-fps" / "engines-pixel-fallback"
DEFAULT_BESTSOURCE = (
    Path.home()
    / ".cache"
    / "resolve-fps"
...
DEFAULT_RVE_RESTORATION_MODEL = Path(
    os.environ.get(
...
def run_ffmpeg_interpolation(
    ...
    if output.exists() and not arguments.force:
        raise RuntimeError(f"Output already exists; use --force to replace it: {output}")
    from resolve_editor.export_process import run_ffmpeg
    from resolve_editor.export_types import ExportProgress
    from resolve_editor.interpolation import build_ffmpeg_interpolation_command
    ...
    try:
        with tempfile.TemporaryDirectory(
            ...
            if render_rate != target_rate or render_frames != target_frames:
                from resolve_editor.export_process import run_ffmpeg
                from resolve_editor.export_types import ExportExecutionError, ExportProgress
...
def run_restoration_rve(
    ...
    *,
    ...
    try:
        with tempfile.TemporaryDirectory(
            prefix="resolve-rve-restoration-",
            dir=output.parent,
        ) as temporary_directory:
            cwd = Path(temporary_directory)
            rve_video = cwd / "rve-video.mp4"
